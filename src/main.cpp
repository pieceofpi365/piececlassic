...
#if defined(NDEBUG)
# error "PieceOfPi cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfa8cacb2a5305ac8e292d9ea0c0f9960ee287afffe9528f9030501ae19688197");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PieceOfPi: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PieceOfPi Signed Message:\n";
...
{
    // PieceOfPi: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 2178892); // PieceOfPi: 2.178892m blocks in ~108.08 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 54.11 * 24 * 60 * 60; // PieceOfPi: 54.11 days
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PieceOfPi: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PieceOfPi: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x37681a2fa1f3e52236ba5f2a12988139981aefe5d581caf4f76b81eb176c77e9");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "20/Dec/2020 Have a Piece!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040c3ce5eaa80a2094537466b1b67ae28364bd3e3a16e46a212707aaad75dd17cb6985a0d145a7de482ca28ee799ffaf5538fce3ad618ad003cee01031814dc18d") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1608505995;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085325630;

        if (fTestNet)
        {
            block.nTime    = 1608505988;
            block.nNonce   = 385699744;
        }
...
printf("block.GetHash = %s\n", block.GetHash().ToString().c_str());
        ...
        assert(block.hashMerkleRoot == uint256("0x0b46fd38e3675b111971ec75065b1654ace8f7f1244557a052317b7b92203d7e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xcl, 0xb0, 0xde }; // PieceOfPi: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PieceOfPiMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PieceOfPiMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PieceOfPiMiner(CWallet *pwallet)
{
    printf("PieceOfPiMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("piece-miner");
    ...
    try { loop {
        ...
        printf("Running PieceOfPiMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PieceOfPiMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PieceOfPiMiner, pwallet));
}
